#!/usr/bin/env python3
"""Generate HTML report from world_cache.json."""
import json
from collections import Counter
from datetime import datetime
from pathlib import Path

CACHE  = Path("world_cache.json")
OUTPUT = Path("world_report.html")
...
def country_flag(country):
    return REGION_FLAGS.get(country, "")
...
def score_detail_section(score_detail):
    if not score_detail:
        return ('<div class="sd-empty">Score composite non disponible — '
                'relancez <code>python world_screener.py --refresh</code>.</div>')
    cells = ""
...
for i, r in enumerate(records):
    ...
    country_td = f'{flag} <span class="country-name">{country}</span>'
    ...
    <tr class="main-row" onclick="toggleDetail({i})" style="cursor:pointer">
      ...
      <td class="country-cell">{country_td}</td>
...
sort_notice = ("Classé par score composite pondéré (0-100)"
               if has_composite else
               "Score composite non disponible — lancez "
               "<code>python world_screener.py --refresh</code> pour calculer "
...
enrich_notice_txt = (f"&#10003; {enriched}/{len(records)} actions enrichies avec les cabinets"
                     if enriched > 0 else
                     "Lancez <code>python world_screener.py --enrich</code> "
...
<title>World Screener — Top Capitalisations Mondiales</title>
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  body {{ font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
          background: #0f172a; color: #e2e8f0; font-size: 13px; }}

  header {{ background: linear-gradient(135deg, #1a1f3a 0%, #0f172a 100%);
  ...
  .sort-notice {{ background: #a78bfa20; border: 1px solid #a78bfa40; color: #c4b5fd; }}
  ...
  .main-row.open {{ background: #16132a; border-bottom: none; }}
  ...
  .name-cell .meta {{ color: #64748b; font-size: 11px; margin-top: 2px; }}
  .country-cell {{ white-space: nowrap; color: #94a3b8; font-size: 12px; }}
  .country-name {{ color: #64748b; }}
  ...
  .toggle-btn.active {{ background: #a78bfa20; border-color: #a78bfa; color: #a78bfa; }}
  .no-btn {{ color: #334155; font-size: 12px; }}

  .detail-row td {{ padding: 0; background: #0c0d1f; border-bottom: 2px solid #1a1f3a; }}
  ...
  .sd-title {{ font-size: 11px; color: #a78bfa; text-transform: uppercase;
...
<header>
  <h1>World Screener &mdash; <span>Top Capitalisations Mondiales</span></h1>
  <p>US &bull; Europe (UE, UK, Suisse) &bull; Asie-Pacifique &bull;
     Score composite multi-facteurs pondéré &bull;
...
<footer>
  World Screener &bull; Score composite : Qualité(35%) · Valorisation(25%) ·
